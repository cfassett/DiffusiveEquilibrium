...
def hohocratersize(impradius):
    ...
    # Relies on scaling params from scalingparams.py
...
def makefullPF(minsizekmlog10,maxsizekmlog10, lstep):
    #Returns the fullPF in meters with frequencies
    size=minsizekmlog10
    sizes=[]
    freqs=[]
    ...
    while size<maxsizekmlog10:
        sizekm=10.0**size
        sizes.append(sizekm)  
        cumfreq=cratcalc(sizekm)
        freqs.append(cumfreq)    
        size=size+lstep
 
    return sizes, freqs  #sizes in km
...
def main():
    try:
        ...
        minsizekmlog10=-3
        maxsizekmlog10=-0
        lstep=0.15051499783199
        diams_km, freqs=makefullPF(minsizekmlog10-lstep,maxsizekmlog10, lstep/10.0)     
        diameterslog=[log10(el) for el in diams_km]
        freqslog=[log10(el) for el in freqs]
        size=minsizekmlog10+lstep/2.0        
        while size<maxsizekmlog10:
            sizelower=(size-lstep/2.0)
            sizeupper=(size+lstep/2.0)
            fluxintlogl=np.interp(sizelower, diameterslog, freqslog)        
            fluxintlogu=np.interp(sizeupper, diameterslog, freqslog)   
            
            diff=10.0**(fluxintlogl)-10.0**(fluxintlogu)
            print 10**(size+3.0),diff
            dsizes.append(1000.0*10**(size))  #convert km to m        
            dfreq.append(diff)  
            size=size+lstep
        ...
        ax1.set_ylabel("Differential freq. (# per $km^2$ per bin)")
        
        ax1.scatter(dsizes, dfreq)     
