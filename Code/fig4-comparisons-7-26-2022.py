...
def man(option, opt, value, parser):
    ...
    print >>sys.stderr, '''\
This program plots the basic form of Fig. 4.
'''
    sys.exit()
...
def main():
    try:
        ...
        plt.yscale('log')
        plt.xlim([1.0,1100.0])
        plt.ylim([1.0e-3,10.0])
        
        plt.xscale('log')
        plt.xlabel("Crater Diameter (m)")
        plt.ylabel("Kappa $(m^2/My)$")

        plt.plot(sizes, kTrask, label='Trask')
        plt.plot(sizes, kHart, label='Hartmann')
        
        sizesK=[8,1000,5,20]
        kLit=[0.109,5.5,0.03,0.4]
        plt.scatter(sizesK, kLit)   
        
        sizeDu=[500,1100]
        kDu=[2.676777543,5.572696271]
        plt.plot(sizeDu, kDu,linestyle='dotted', label='Du et al.')   
