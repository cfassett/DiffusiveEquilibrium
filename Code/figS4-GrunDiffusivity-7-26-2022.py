...
def man(option, opt, value, parser):
    ...
    print >>sys.stderr, '''\
This program uses  a bunch of different morphometry models and produces a plot of the diffusivity that you need given those models.
'''
    sys.exit()
...
def diffusetothreshQ(diam,visibilitythreshold):
    domainsizepx=400
    sscale=diam/100.0
...
def diffusetothreshO(diam,visibilitythreshold):
    domainsizepx=400
    sscale=diam/100.0
...
def diffusetothreshP(diam,visibilitythreshold):
    domainsizepx=400
    sscale=diam/100.0
...
def main():
    try:
        ...
        while size<maxsizekmlog10:
            ...
            kT_thissize=diffusetothreshO(sizekm*1000.0,visibilitythreshold)                
            k_thissizeTO=kT_thissize/(eqtimeton(yearstotrask)/1.0e6)
            kTraskO.append(k_thissizeTO)

            kT_thissize=diffusetothreshP(sizekm*1000.0,visibilitythreshold)                
            k_thissizeTP=kT_thissize/(eqtimeton(yearstotrask)/1.0e6)
            kTraskP.append(k_thissizeTP)
            
            kT_thissize=diffusetothreshQ(sizekm*1000.0,visibilitythreshold)                
            k_thissizeTQ=kT_thissize/(eqtimeton(yearstotrask)/1.0e6)
            kTraskQ.append(k_thissizeTQ)
            ...
            print(sizemid)
            out.write(str(sizemid)+","+str(k_thissizeT)+","+str(k_thissizeTO)+","+str(k_thissizeTP)+","+str(k_thissizeTQ)+"\n")
            size=size+lstep
