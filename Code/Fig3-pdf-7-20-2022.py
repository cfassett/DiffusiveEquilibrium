...
def numtrask(lowdiamkm, updiamkm, surfacearea):
...
def cratcalc2(size_km, size_kmupper,lstep,surfacearea):
    ...
    if (size<10.0):
        if sulog>grundiameterslog[-1]:   #In range where need to interpolate between Neukum at 10m and whatever the maximum Grun diameter is
            ...
            interpfreqs=[cratcalc2(10**(grundiameterslog[-1]-lstep-epsilon-3.0),10**(grundiameterslog[-1]-epsilon-3.0),lstep,surfacearea),(10**log10(npfcalc(0.01))-10**log10(npfcalc(10**(-2.0+lstep))))]
...
def getdiams(surfacearea, lowersizethreshold=(10**-2.3), uppersizethreshold=10.0**-1.0, logsizestep=0.1):
    ...
    while (upperbinsize>loglower):   
        ...
        expectednumberinthisbin=numtrask(lowersizethreshold, uppersizethreshold, surfacearea)
        if expectednumberinthisbin < 0: expectednumberinthisbin=0
...
def main():
    try:
        ...
        while size<maxsizekmlog10:
            ...
            oneyearfreqatthissize=cratcalc2(sizekm,sizeupperkm,lstep, surfacearea)            
            yearstotrask=(numtrask(sizekm,sizeupperkm,surfacearea))/oneyearfreqatthissize
            ...
            newdiams=getdiams(surfacearea, lowersizethreshold=sizekm, uppersizethreshold=sizeupperkm, logsizestep=lstep)         
            ...
            print "C:",size, surfacearea, oneyearfreqatthissize*yearstotrask
            size=size+lstep                        
