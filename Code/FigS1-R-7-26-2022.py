...
def main():
    try:
        ...
        while size<maxsizekmlog10:
            ...
            rvalue=((sizemid*1000)**3.0)*((cratcalc2(sizekm,sizeupperkm,lstep))/lstep)
            sizes.append(1000.0*10**(size+lstep/2.0))  #convert km to m        
            freqs.append(rvalue)  
            print sizekm, rvalue
            size=size+lstep  
        ...
        plt.ylabel("Relative crater frequency (R value)")
        ax1.set_aspect('equal')
        plt.scatter(sizes, freqs)
        #plt.legend()
        #plt.show()
        plt.savefig('FigS1R.png', dpi=300)
