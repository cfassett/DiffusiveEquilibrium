...
def man(option, opt, value, parser):
    ...
    print >>sys.stderr, '''\
This program is Figure 1: equilibrium lifetimes (hart, trask), assuming Grun+Neukum PFs vs size.
'''
    sys.exit()
...
def cratcalc(size_km):
    ...
    if size<10.0:
        slog=log10(size)
        if slog>grundiameterslog[-1]:            
            fluxintlog=np.interp(slog,(grundiameterslog[-1],1.0),(grunfluxeslog[-1],log10(npfcalc(0.01))))            
        else:
