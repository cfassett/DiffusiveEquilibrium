...
def cratcalc2(size_km, size_kmupper,lstep=0.001):
    ...
    if (size<10.0):
        if slog>grundiameterslog[-1]:   #In range where need to interpolate between Neukum at 10m and whatever the maximum Grun diameter is
            ...
            interpfreqs=[cratcalc2(10**(grundiameterslog[-1]-lstep-3.0),10**(grundiameterslog[-1]-3.0)),(10**log10(npfcalc(0.01))-10**log10(npfcalc(10**(-2.0+lstep))))]            
            ...
            #print (slog+sulog)/2.0, dfluxintlog, dflux
            ...
            #dflux1=cratcalc2(10.0**(grundiameterslog[-1]+3.0),10.0**(sulog+3.0))
            #dflux2=cratcalc2(10**(slog-3.0),10.0**(grundiameterslog[-1]-3.0))
            #dflux=dflux1+dflux2
            dflux=np.nan
    ...
    else:  #this branch should never happen, unless there is a bug.
        globalfreq=np.nan
...
def main():
    try:
        ...
        while size<maxsizekmlog10:
            ...
            print 10.0**(size+0.5*lstep), oneyearfreqatthissize, eqtimeton(yearstohart), eqtimeton(yearstotrask)
            eqtimes.append(eqtimeton(yearstohart)/1.0e6)
            eqtimest.append(eqtimeton(yearstotrask)/1.0e6)
        ...
        plt.ylabel("Equil. Lifetime (My)")

        plt.scatter(esizes, eqtimes, label='Hartmann')
        plt.scatter(esizes, eqtimest, label="Trask")
        plt.legend()
        plt.show()
